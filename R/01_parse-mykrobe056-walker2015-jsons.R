...
# Parse Mykrobe 0.5.6 with walker-2015 panel
...
# Write some output
# Report as is
write.csv(report,file=paste("output-Walker056-",opt$reportfile,".csv",sep=""), row.names = F)
...
temp %>% 
  select(file, 
         ...
         lineage_depth) %>%
  distinct() %>%
  write.csv(file=paste("output-Walker056-",opt$speciationfile,".csv",sep=""), row.names = F)

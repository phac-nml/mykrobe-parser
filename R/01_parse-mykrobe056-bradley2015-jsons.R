...
option_list = list(
  ...
  make_option(c("-D", "--depth"), 
              type="integer", 
              default="", 
              help="Minimum depth of coverage used to run Mykrobe", 
              metavar="integer"),
...
report <- 
  report %>% 
  mutate(
    Mykrobe_Pipeline_Version = params["Mykrobe_Pipeline_Version"],
    Mykrobe_min_depth = params["Mykrobe_min_depth"],
    Mykrobe_min_conf = params["Mykrobe_min_conf"],
...
# Write some output
# Report as is
write.csv(report,file=paste("output-Bradley056-",opt$reportfile,".csv",sep=""), row.names = F)
...
temp %>% 
  select(file, 
         ...
         lineage_depth) %>%
  distinct() %>%
  write.csv(file=paste("output-Bradley056-",opt$speciationfile,".csv",sep=""), row.names = F)
